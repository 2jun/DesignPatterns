...
public class DBConnectionPool extends Pool {
    ...
    private DBConnectionPool() {
        ...
        {
            init();
            for (int i = 0; i < normalConnect; i++) { // 初始normalConn 个连接
    ...
    private Connection newConnection() {
        ...
        try {
            ...
            System.out.println("无法创建这个URL 的连接" + url);
            return null;
        }
        return con;
    }
    ...
    public synchronized Connection getConnection() {
        ...
        } else if (maxConnect == 0 || checkedOut < maxConnect) { // 没有空闲连接且当前连接小于最大允许值,最大值为0 则不限制
                    con = newConnection();
        }
        if (con != null) { // 当前连接数加1
            checkedOut++;
